...
# 
# Part 2: floor is at 158 , sandmap x-value increased to 1000 to accomodate more sand 
###################################################################################

sandmap = [['.' for x in range(1000)] for y in range(160)]
...
def SandFlow():
    ...
    try:
        while(1): #while abyss not reached
            ...
            sand_x = 500 
            sand_y = 0 
            if ( (sandmap[1][500] != '.') and (sandmap[1][499] != '.') and (sandmap[1][501] != '.') ):
                print("Sand Source is blocked!!")
                #MapPrinter() # for fun, remove if required 
                print(sand_count) # last sand blocks entry = at rest 
                break
            while (1): # while sand can still move
                if sand_y == 157: # reached the floor 
                    sandmap[sand_y][sand_x] = '0'
                    break
                elif sandmap[sand_y+1][sand_x] == '.':
                    sand_y += 1 
    ...
    except IndexError: # part2: should not reach here anymore 
...
#Build map based on entry 
    sandmap[0][500] = 'S' # mark start of sand entry 
    for i in range(len(sandmap[158])): #simulate the floor 
        sandmap[158][i] = "#"
    for line in Lines: #iterate through all lines in entry.txt
        a = line.strip().split(' -> ')
        for index in range(1,len(a)): #iterate through all coordinates in a line, starting from 2nd entry, since we will be comparing coo
            if a[index].split(',')[0] == a[index-1].split(',')[0]: #if x-value is same , draw '#' from prev coordinate until it reaches this coordinate 
                ...
                while current_tile != int(a[index-1].split(',')[1]): 
                    if current_tile < int(a[index-1].split(',')[1]):
                        ...
                        current_tile -= 1
                        sandmap[current_tile][int(a[index].split(',')[0])] = '#'
            else: #y-value is same, draw '#' from prev coordinate until it reaches next coordinate 
