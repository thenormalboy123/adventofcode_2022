...
if __name__ == '__main__':
    ...
    for line in Lines:
        if (line_number < 9):
            ...
            for i in range(len(movement_instruction)-1):
                movement_instruction[i+1] = int(movement_instruction[i+1].strip())
            
            if (len(stack[movement_instruction[2] - 1]) < movement_instruction[1]):
                movement_instruction[1] = len(stack[movement_instruction[2] - 1])
            
            for moves in range(movement_instruction[1]):
                moved_item = stack[movement_instruction[2] - 1][len(stack[movement_instruction[2] - 1]) - 1] 
...
# ALPHABET INDEX: 1 5 9 13 17 21 25 29 33